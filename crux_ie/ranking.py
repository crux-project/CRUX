...
def ranking_list():
    for datacard in db["datacard"].find():
        condition = {"dataID": datacard["_id"],
                     "performance.F1_score": {"$exists": True},
                     "groundtruth": {"$ne": "output.peaklist"}
                     }
        order_list = db.testcard.find(
            condition, {"_id": 0, "modelID": 1, "performance": 1}

...
def load_model(path='./ast2vec/ast2vec.pt'):
    """ Loads the ast2vec version 2 model from the given path.
    ...
    -------
    class rtgae2.RTGAE
        An instance of the ast2vec version 2 model.

    """
    # initialize a new autoencoder instance
    model = rtgae2.RTGAE(python_ast_utils.grammar, dim=DIM_)
...
def encode_trees(model, trees, verbose=False):
    ...
    Paramters
    ---------
    model: class rtgae2.RTGAE
        An instance of the ast2vec model.
    trees: list
        A list of tree.Tree objects, each representing one syntax tree.
    verbose: bool (default = False)
        If set to true, we show a progress bar.

    Returns
    -------
    X: ndarray of size len(list) x DIM_
...
def decode_points(model, X, max_size=100, verbose=False):
    ...
    -------
    trees: list
        A list of decoded tree.Tree objects, each representing a syntax tree.
    ...
    for i in progbar(range(X.shape[0])):
        ...
        # decode the point
        tree = model.decode(x, max_size=max_size)
        trees.append(tree)
...
def interpolation_grid(model, start_tree, target_tree, grid_size=11, X=None, max_size=100):
    ...
    model: class rtgae2.RTGAE
        An instance of the ast2vec model.
    start_tree: tree.Tree OR int
        The tree where we start the interpolation, represented as a
        tree.Tree object. If the X argument is given, this can also
        be just the index of the start data point.
    target_tree: tree.Tree OR int
        The tree where we end the interpolation, represented as a
        tree.Tree object. If the X argument is given, this can also
        be just the index of the target data point.
    grid_size: int (default = 11)
        The number of regular samples between start and target.
    X: ndarray of size m x DIM_ (default = None)
    ...
    Returns
    -------
    points: ndarray of size grid_size x DIM_
        ...
        grid_size x grid_size x DIM_ where the start is at location
    ...
    if X is None:
        ...
        x = model.encode(start_tree).detach().numpy()
        y = model.encode(target_tree).detach().numpy()

        # initialize output
        points = np.zeros((grid_size, DIM_))
        ...
        for i in range(grid_size):
            ...
            # decode it
            tree = model.decode(torch.tensor(points[i, :], dtype=torch.float), max_size=max_size)
        ...
        # initialize output
        points = np.zeros((grid_size, grid_size, DIM_))
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                ...
                k += 1
                # decode it
                tree = model.decode(torch.tensor(points[i, j, :], dtype=torch.float), max_size=max_size)
                # check if this tree already occured
                l = 0
                while l < len(trees):
                    if tree == trees[l]:
...
def interpolation_plot(model, start_tree, target_tree, grid_size=11, X=None, max_size=100, variable_classifier=None,
                       plot_code=3):
    ...
    _, grid, grid_trees = interpolation_grid(model, start_tree, target_tree, grid_size=grid_size, X=X,
                                             max_size=max_size)
    ...
    if plot_code is not None and plot_code > 0:
        ...
        for k in freq_trees:
            ...
            if variable_classifier is not None:
                variable_classifier.predict(tree)
            ast = python_ast_utils.tree_to_ast(tree)
...
def traces_plot(start, target, traces, X, trees=None, plot_code=0):
    ...
    trees: list (default = None)
        A list of tree.Tree objects, representing the syntax trees of student
        data. This is necessary to draw example student code. Otherwise, only
    ...
    # draw all arrows
    plt.quiver(Xarr[:, 0], Xarr[:, 1], Varr[:, 0], Varr[:, 1], Carr, angles='xy', scale_units='xy', scale=1.,
               width=0.005)
    ...
    if plot_code > 0:
        ...
        for k in freq_trees:
            ast = python_ast_utils.tree_to_ast(trees[k])
            src = astor.to_source(ast)
            if src == '':
                src = '<empty program>'
            plt.text(Xlo[k, 0] + text_offset, Xlo[k, 1], src, bbox=dict(facecolor='white', alpha=0.7),
...
def dynamical_system_plot(W, start_tree, target_tree, X, grid_size=11, arrow_scale=10., step_size=1., max_steps=10,
                          model=None, max_size=100, variable_classifier=None):
    ...
    model: class rtgae2.RTGAE (default = None)
    ...
    """
    # do the interpolation
    points, grid, grid_trees = interpolation_grid(model, start_tree, target_tree, grid_size=grid_size, X=X,
                                                  max_size=max_size)

    # compute the dynamical system predictions for all points in the grid
    x = points[0, grid_size // 2, :]
    y = points[-1, grid_size // 2, :]
    Xgrid = points.reshape((grid_size ** 2, DIM_))
    ...
    plt.quiver(Xgridlo[:, 0], Xgridlo[:, 1], Ugridlo[:, 0], Ugridlo[:, 1], angles='xy', scale_units='xy',
               scale=arrow_scale, width=0.005, color='red')
    ...
    if step_size > 0.:
        ...
        # plot
        plt.quiver(Xtracelo[:, 0], Xtracelo[:, 1], Vtracelo[:, 0], Vtracelo[:, 1], angles='xy', scale_units='xy',
                   scale=1., width=0.005, color='blue')
        ...
        if model is not None:
            ...
            for t in range(Xtrace.shape[0]):
                ...
                if src == '':
                    src = '<empty program>'
                plt.text(Xtracelo[t, 0], Xtracelo[t, 1] + text_offset, src, bbox=dict(facecolor='white', alpha=0.7),
...
def cluster_plot(start, target, X, Y, means=None, traces=None, model=None, max_size=100, variable_classifier=None):
    ...
    means: ndarray of size K x DIM_ (default = None)
    ...
    if means is not None:
        ...
        if model is not None:
            ...
            for k in range(means.shape[0]):
                tree = cluster_trees[k]
                variable_classifier.predict(tree) if variable_classifier is not None else None
                ast = python_ast_utils.tree_to_ast(tree)
                src = astor.to_source(ast)
                if src == '':
                    src = '<empty program>'
                plt.text(meanslo[k, 0] + text_offset, meanslo[k, 1], src, bbox=dict(facecolor='white', alpha=0.7),
                         horizontalalignment='left', verticalalignment='bottom')
